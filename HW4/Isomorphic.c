...
int Isomorphic( Tree T1, Tree T2 )
{
    ...
    if((T1->Right == NULL && T2->Right == NULL)) return Isomorphic(T1->Left,T2->Left);
    if((T1->Left == NULL && T2->Right == NULL)) return Isomorphic(T1->Right,T2->Left);
    if((T1->Right == NULL && T2->Left == NULL)) return Isomorphic(T1->Left,T2->Right);
    if((T1->Left->Element == T2->Left->Element)) return (Isomorphic(T1->Left,T2->Left))&&(Isomorphic(T1->Right,T2->Right));
